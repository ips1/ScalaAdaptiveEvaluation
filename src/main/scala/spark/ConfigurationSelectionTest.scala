...
import spark.data.RecordDatasetGenerator
import spark.queries.ReduceByKeyQuery
...
object ConfigurationSelectionTest {
  ...
  def createSparkSession(config: SparkAdaptiveConfiguration): SparkSession =
    createBaseSparkSession()
      .config("spark.sql.adaptive.enabled", config.adaptiveEnabled)
      .config("spark.sql.adaptive.shuffle.targetPostShuffleInputSize", config.targetPostShuffleInputSize)
      .config("spark.sql.adaptive.minNumPostShufflePartitions", config.minNumPostShufflePartitions)
      .getOrCreate()
